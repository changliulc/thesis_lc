...
def plot_preview(t_plot: np.ndarray, dbar_plot: np.ndarray, pcar_plot: np.ndarray) -> dict[str, Path]:
    ...
    ax1.tick_params(labelsize=12, length=5, width=1.0, top=True, right=True)
    ax1.set_ylabel("磁场差分值 (nT)", fontsize=14, fontproperties=mixed_font(14))
    ...
    ax2.tick_params(labelsize=12, length=5, width=1.0, top=True, right=True)
    ax2.set_ylabel("有车概率 Pcar(k)", fontsize=14, fontproperties=mixed_font(14))
    ax2.set_xlabel("时间 (s)", fontsize=14, fontproperties=mixed_font(14))
