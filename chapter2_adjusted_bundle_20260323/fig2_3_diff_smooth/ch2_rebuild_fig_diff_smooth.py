...
AXES_LAYOUT = [
    ("axes_1", "matplotlib.axis_2", "$B$", "三轴原始 B_l"),
    ("axes_2", "matplotlib.axis_4", "$D$", "一阶差分 D_l"),
    ("axes_3", "matplotlib.axis_6", "$\\overline{D}$", "平滑差分 Dbar_l"),
]
# Follow the reference Matplotlib-style figure:
# blue for x, orange-red for y, and warm yellow for z.
MATLAB_LINE_COLORS = ["#1f77b4", "#d95319", "#edb120"]
AXES_WIDTH_SHRINK = 0.83
PANEL_TITLE_FONTSIZE = 14
AXIS_LABEL_FONTSIZE = 13
TICK_LABEL_FONTSIZE = 12
LEGEND_FONTSIZE = 14
LINE_WIDTH_SCALE = 1.55
...
def first_comment_text(node) -> str | None:
    for child in node.childNodes:
        if child.nodeType == child.COMMENT_NODE:
            txt = child.data.strip()
            if txt:
                return txt
        if child.nodeType == child.ELEMENT_NODE:
            txt = first_comment_text(child)
            if txt:
                return txt
    return None
...
def rebuild_from_reference(reference_svg: Path, out_dir: Path, dpi: int) -> dict[str, Path]:
    ...
    for idx, (axes_id, y_axis_id, ylabel, title) in enumerate(AXES_LAYOUT):
        ...
        handles = []
        for j, (label, trace) in enumerate(zip(LINE_LABELS, parse_direct_line_traces(axes_node), strict=True)):
            x_data = x_map(trace.x_px)
            y_data = y_map(trace.y_px)
            handle = ax.plot(
                x_data,
                y_data,
                color=MATLAB_LINE_COLORS[j],
        ...
        ax.set_title(
            ...
            pad=8,
            fontproperties=mixed_font(PANEL_TITLE_FONTSIZE),
        )
        ax.set_ylabel(ylabel, fontsize=AXIS_LABEL_FONTSIZE)
        ax.grid(True, color="#D0D0D0", linewidth=0.9, alpha=0.55)
        ...
        if idx < len(AXES_LAYOUT) - 1:
            ...
            ax.set_xlabel(
                ...
                fontproperties=mixed_font(AXIS_LABEL_FONTSIZE),
            )
