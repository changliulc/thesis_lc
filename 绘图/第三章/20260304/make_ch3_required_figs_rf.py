...
FEATURE_LABELS = {
    "duration_s": {"zh": "事件时长", "en": "Duration"},
    "mag_peak": {"zh": "模值峰值", "en": "Mag peak"},
    "mag_energy": {"zh": "模值能量", "en": "Mag energy"},
    "mag_rms": {"zh": "模值RMS", "en": "Mag RMS"},
    "mag_mean": {"zh": "模值均值", "en": "Mag mean"},
    "mag_std": {"zh": "模值标准差", "en": "Mag std"},
    "mag_iqr": {"zh": "模值四分位距", "en": "Mag IQR"},
    "mag_ptp": {"zh": "模值峰峰值", "en": "Mag PTP"},
    "x_std": {"zh": "X轴标准差", "en": "X std"},
    "y_std": {"zh": "Y轴标准差", "en": "Y std"},
    "z_std": {"zh": "Z轴标准差", "en": "Z std"},
    "x_ptp": {"zh": "X轴峰峰值", "en": "X PTP"},
    "y_ptp": {"zh": "Y轴峰峰值", "en": "Y PTP"},
    "z_ptp": {"zh": "Z轴峰峰值", "en": "Z PTP"},
    "x_absmean": {"zh": "X轴绝对均值", "en": "X abs mean"},
    "y_absmean": {"zh": "Y轴绝对均值", "en": "Y abs mean"},
    "z_absmean": {"zh": "Z轴绝对均值", "en": "Z abs mean"},
    "x_energy": {"zh": "X轴能量", "en": "X energy"},
    "y_energy": {"zh": "Y轴能量", "en": "Y energy"},
    "z_energy": {"zh": "Z轴能量", "en": "Z energy"},
    "corr_xy": {"zh": "XY相关系数", "en": "Corr XY"},
    "corr_xz": {"zh": "XZ相关系数", "en": "Corr XZ"},
    "corr_yz": {"zh": "YZ相关系数", "en": "Corr YZ"},
    "peak_pos_ratio": {"zh": "峰值位置比例", "en": "Peak pos ratio"},
}
...
def plot_amp_boxplot(df_event: pd.DataFrame, out_no_ext: str, lang: str = "zh", amp_metric: str = "energy") -> None:
    ...
    if amp_metric == "energy":
        ax.set_yscale("log")
        ax.set_ylabel("模值能量 / a.u." if lang == "zh" else "Magnitude energy / a.u.")
...
def plot_feature_importance(df_imp: pd.DataFrame, out_no_ext: str, lang: str = "zh", topn: int = 12) -> None:
    ...
    ax.set_xlabel("重要性（随机森林）" if lang == "zh" else "Importance (validation permutation importance)")
    ax.set_title("端侧统计特征重要性排序" if lang == "zh" else "Ranking of edge statistical features")
    fig.tight_layout()
    save_pdf_png(fig, out_no_ext)
    plt.close(fig)
...
def plot_training_curve(df_hist: pd.DataFrame, out_no_ext: str, best_epoch: int, lang: str = "zh") -> None:
    ...
    ax1.plot(df_hist["epoch"], df_hist["train_loss"], linewidth=1.5, label="train loss")
    ax1.plot(df_hist["epoch"], df_hist["val_loss"], linewidth=1.5, label="val loss")
    ...
    ax2.plot(df_hist["epoch"], df_hist["val_acc"], linewidth=1.5, label="val acc")
    ax2.plot(df_hist["epoch"], df_hist["val_macro_f1"], linewidth=1.5, label="val macro-F1")
