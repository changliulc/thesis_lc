...
def main(mat_path):
    ...
    _=dtw_warp_mv(seq4[0], tmpl4[0], w, 0.0)
    ...
    for i in range(len(y)):
        ...
        if d2<bestd:
            bestc=2
        aligned=dtw_warp_mv(seq4[i], tmpl4[bestc], w, 0.0)
        Xfeat[i]=aligned.T.reshape(-1)
    print('align_build_s', round(time.time()-t1,3))
    ...
    t2=time.time()
    W,b,it,f1v=train_softmax(Xtr,ytr,Xva,yva,lam=1e-3,lr=0.3,max_iter=250,patience=25,seed=42)
