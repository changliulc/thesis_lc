...
def build_dtw_clsmin(X_raw, y, idx_tr, wR=0.15, step=0.0):
    # 1 template per class = mean
    C=int(y.max())+1
    L=X_raw.shape[2]
    ...
    for i in range(N):
        ...
        for c in range(C):
            ...
            if d<bestd:
                bestd=d
                bestc=c
        ali=dtw_warp_mv(X_raw[i].T, templates[bestc].T, w, step)
        X_out[i]=ali.T
    return X_out
...
def main():
    ...
    for wR,step in configs:
        t0=time.time()
        Xal_raw=build_dtw_clsmin(X_raw,y,idx_tr,wR=wR,step=step)
        mu2,sd2=compute_norm_stats(Xal_raw, idx_tr)
        Xal=normalize(Xal_raw,mu2,sd2)
        build_s=time.time()-t0
        acc_va,f1_va,acc_te,f1_te=train_model(Xal,y,idx_tr,idx_va,idx_te,in_ch=4)
        print('DTWclsmin', 'wR',wR,'step',step,'build_s',round(build_s,2),'val',round(acc_va,4),round(f1_va,4),'test',round(acc_te,4),round(f1_te,4), flush=True)
        if (best is None) or (f1_va>best[0]):
            best=(f1_va,(wR,step,acc_va,acc_te,f1_te))
