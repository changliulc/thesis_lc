import os, random, time, math, struct, zlib
from pathlib import Path
import numpy as np

# thread safety (avoid deadlocks)
...
def build_baseline_data(xyz_list, L):
    ...
    for i,xyz in enumerate(xyz_list):
        seq=make_4ch_seq(xyz,L)  # (L,4)
        X[i]=seq.T
    return X


def build_templates(X_raw, y, idx_tr, templates_per_class=1, seed=42):
    rng=np.random.default_rng(seed)
    # X_raw: (N,4,L) unnormalized
    C=int(y.max())+1
    templates=[]
    tpl_labels=[]
    for c in range(C):
        ...
        if templates_per_class==1:
            tpl = X_raw[ids].mean(axis=0)
            templates.append(tpl)
            tpl_labels.append(c)
        else:
            # pick random exemplars
            pick = rng.choice(ids, size=min(templates_per_class, len(ids)), replace=False)
            for pid in pick:
                templates.append(X_raw[pid])
                tpl_labels.append(c)
    templates=np.stack(templates, axis=0)  # (T,4,L)
    tpl_labels=np.array(tpl_labels, np.int64)
    return templates, tpl_labels


def build_dtw_clsmin(X_raw, y, idx_tr, wR=0.15, step=0.0, seed=42):
    # templates: 1 per class (mean)
    templates, tpl_labels = build_templates(X_raw, y, idx_tr, templates_per_class=1, seed=seed)
    C=int(y.max())+1
    L=X_raw.shape[2]
    ...
    for i in range(N):
        # choose best class by mag DTW distance
        mi = X_raw[i,3]
        bestc=0
        bestd=1e30
        for c in range(C):
            ...
            if d < bestd:
                bestd=d
                bestc=c
        aligned = dtw_warp_mv(X_raw[i].T, templates[bestc].T, w, step)  # (L,4)
        X_al[i] = aligned.T
    return X_al


def build_dtw_multi(X_raw, y, idx_tr, templates_per_class=3, wR=0.15, step=0.0, seed=42):
    templates, tpl_labels = build_templates(X_raw, y, idx_tr, templates_per_class=templates_per_class, seed=seed)
    L=X_raw.shape[2]
    w=max(1,int(round(wR*L)))
    T=templates.shape[0]
    ...
    for i in range(N):
        ...
        Xi = X_raw[i].T  # (L,4)
        for t in range(T):
            aligned = dtw_warp_mv(Xi, templates[t].T, w, step)  # (L,4)
            feats.append(aligned.T)  # (4,L)
        X_out[i] = np.concatenate(feats, axis=0)
    return X_out

# ---------------- main ----------------

def main(mat_path='/mnt/data/3f82e08c-9373-4134-b811-015b645b3a9e.mat', L=176, seed=42):
    set_seed(seed)
    ...
    print('loaded', len(y), 'split', len(idx_tr), len(idx_va), len(idx_te), 'time_s', round(time.time()-t0,3))

    # ---- baseline ----
    t1=time.time()
    X_raw = build_baseline_data(xyz_list, L)  # (N,4,L)
    mu,sd = compute_norm_stats(X_raw, idx_tr)
    X = normalize(X_raw, mu, sd)
    print('baseline build_s', round(time.time()-t1,3), 'X', X.shape)

    acc_va,f1_va,acc_te,f1_te = train_model(X, y, idx_tr, idx_va, idx_te, in_ch=4, seed=seed)
    print('BASE CNN: val_acc', round(acc_va,4), 'val_f1', round(f1_va,4), 'test_acc', round(acc_te,4), 'test_f1', round(f1_te,4))

    # ---- DTW-ClsMin ----
    t2=time.time()
    X_al_raw = build_dtw_clsmin(X_raw, y, idx_tr, wR=0.15, step=0.0, seed=seed)
    mu2,sd2 = compute_norm_stats(X_al_raw, idx_tr)
    X_al = normalize(X_al_raw, mu2, sd2)
    print('DTW-ClsMin align_s', round(time.time()-t2,3), 'X', X_al.shape)
    acc_va,f1_va,acc_te,f1_te = train_model(X_al, y, idx_tr, idx_va, idx_te, in_ch=4, seed=seed)
    print('DTW-ClsMin CNN: val_acc', round(acc_va,4), 'val_f1', round(f1_va,4), 'test_acc', round(acc_te,4), 'test_f1', round(f1_te,4))

    # ---- DTW-Multi ----
    t3=time.time()
    X_mu_raw = build_dtw_multi(X_raw, y, idx_tr, templates_per_class=3, wR=0.15, step=0.0, seed=seed)
    mu3,sd3 = compute_norm_stats(X_mu_raw, idx_tr)
    X_mu = normalize(X_mu_raw, mu3, sd3)
    print('DTW-Multi align_s', round(time.time()-t3,3), 'X', X_mu.shape)
    acc_va,f1_va,acc_te,f1_te = train_model(X_mu, y, idx_tr, idx_va, idx_te, in_ch=X_mu.shape[1], seed=seed)
    print('DTW-Multi CNN: val_acc', round(acc_va,4), 'val_f1', round(f1_va,4), 'test_acc', round(acc_te,4), 'test_f1', round(f1_te,4))
