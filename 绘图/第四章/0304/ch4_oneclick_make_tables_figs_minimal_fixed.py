...
@dataclass
class LWCParams:
    """LWC+ baseline parameters (aligned to ch4_oneclick_eval_all.py).

    Notes:
      - Indices in this script are 1-based (consistent with ch4_verify_python.py).
      - ΔF uses L1 distance (sum of absolute differences).
    """

    # θ_park: threshold on ΔF for park/leave
    theta_park: float = 120.0
    # θ_V: counter threshold to handle continuous traffic when stable point is missing
    theta_V: int = 2
    # seek window length after each event (seconds)
    seek_sec: float = 2.13
    # mean window length (points) used when stable point is not found or invalid
    np_mean: int = 25
    # environment baseline update coefficient in FREE
    alpha_env: float = 0.162
    # parking baseline EMA coefficient in OCC
    gamma_park: float = 0.10


def _mean_window_after(B: np.ndarray, k0_1based: int, L: int) -> np.ndarray:
    """Mean of samples after an event, with 1-based starting index.

    Args:
      B: (n,3)
      k0_1based: start position in [1..n]
      L: window length (points)
    Returns:
      mean vector (3,)
    """
    n = int(B.shape[0])
    # convert to 0-based slice
    a = max(0, int(k0_1based) - 1)
    b = min(n, a + int(L))
    if b <= a:
        return B[min(a, n - 1), :].copy()
    return np.mean(B[a:b, :], axis=0)
...
def run_lwc_plus_cached(
    ...
    This implementation keeps the same vehicle-event base (events) and the same
    stability outputs (stableState/meanVec), and only compares the parking/
    occupancy decision logic, so that the comparison focuses on the parking
    detection stage.

    Compared to the simplified version, this function additionally includes:
      1) FREE-state environment baseline update (alpha_env);
      2) continuous-traffic counter (theta_V) when post-event stable point is missing;
      3) OCC-state baseline maintenance with environment compensation (gamma_park);
      4) ΔF implemented with L1 distance (sum of abs diffs).
    """
    ...
    # init environment baseline from the first stable window if exists
    idx0 = np.where(stableState)[0]
    if idx0.size > 0:
        kk = int(idx0[0]) + 1  # 1-based
        S_env = meanVec[kk - 1, :].copy()
    else:
        S_env = np.mean(B[: max(1, params.np_mean), :], axis=0)

    ParkMark = False
    CV = 0
    S_park: Optional[np.ndarray] = None
    open_k_in: Optional[int] = None

    out: List[List[int]] = []

    for e in events:
        k_out = int(e["k_out"])  # 1-based

        # update environment baseline while FREE using last stable before this event
        if (not ParkMark) and (params.alpha_env > 0):
            idx = np.where(stableState[:k_out])[0]  # includes k_out-1
            if idx.size > 0:
                k_last = int(idx[-1]) + 1
                S_cand = meanVec[k_last - 1, :]
                if np.all(np.isfinite(S_cand)):
                    S_env = (1 - params.alpha_env) * S_env + params.alpha_env * S_cand

        # find a stable point after k_out within seek window
        ...
        if stable_found:
            ...
            if not np.all(np.isfinite(mu)):
                mu = _mean_window_after(B, k_out, params.np_mean)
        else:
            k_st = None
            mu = _mean_window_after(B, k_out, params.np_mean)

        # ΔF: L1 distance to environment baseline
        dF = float(np.sum(np.abs(mu - S_env)))

        if not ParkMark:
            if dF >= params.theta_park:
                # stable -> direct park; unstable -> counter
                if stable_found:
                    ParkMark = True
                    open_k_in = k_out
                    S_park = mu.copy()
            ...
            if dF < params.theta_park:
                # leave
                ParkMark = False
                if open_k_in is not None:
                    out.append([int(open_k_in), int(k_out)])
                open_k_in = None
                CV = 0
                # after leaving, re-anchor environment baseline
                S_env = mu.copy()
                S_park = None
            else:
                # still occupied -> update baselines (parking baseline + env compensation)
                if S_park is None:
                    S_park = mu.copy()
                S_park_new = (1 - params.gamma_park) * S_park + params.gamma_park * mu
                # env compensation by delta of park baseline
                S_env = S_env + (S_park_new - S_park)
                S_park = S_park_new
...
@dataclass
class ADTAParams:
    """ADTA-FSM baseline parameters."""

    # baseline update rate
    alpha: float = 0.005
    # threshold scale
    beta: float = 0.005
    # debounce length for entering present state
    n_on: int = 5
    # debounce length for leaving present state
    n_off: int = 5
    # minimal duration of an output segment
    Tmin_sec: float = 4.0
...
def fig_baseline_cmp_f1(summary: pd.DataFrame, out_pdf: str) -> None:
    ...
    method_labels = {"Ours": "本文方法", "LWC": "基线1", "ADTA-FSM": "基线2"}
...
def fig_C_seek_ratio(r_found: float, r_deg: float, out_pdf: str) -> None:
    ...
    ax.bar([0, 1], vals)
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["稳定点可得率", "退化触发率"])
    ax.set_ylim(0, 1)
    ax.set_ylabel("比例")
...
def main() -> int:
    ...
    # ------------------------------
    lwc_params = LWCParams()
    adta_params = ADTAParams(alpha=0.005, beta=0.005, n_on=5, n_off=5, Tmin_sec=args.Tmin)
