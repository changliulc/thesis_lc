...
$\mathbf{B}(k)$ & 第 $k$ 个采样时刻的三轴地磁观测向量（第2章、第3章） \\
...
$\boldsymbol{\varepsilon}(k)$ & 测量噪声及外界干扰向量 \\
$f_s$ & 地磁传感器采样频率 \\
$k_{\mathrm{in}}$ & 事件起始边界索引 \\
$k_{\mathrm{out}}$ & 事件结束边界索引 \\
...
$p_{1,l}(k)$ & 第 $l$ 轴扰动得分 \\
$P_{\mathrm{env}}(k)$ & 融合无车概率 \\
$P_{\mathrm{car}}(k)$ & 融合有车概率 \\
$\rho_0,\,\rho_1$ & 三轴概率融合的相关性修正系数 \\
$\theta_{\mathrm{arr}}$ & 到达检测概率阈值 \\
$\theta_{\mathrm{lea}}$ & 离开检测概率阈值 \\
$N_{\mathrm{arr}}$ & 到达连续计数阈值 \\
$N_{\mathrm{lea}}$ & 离开连续计数阈值 \\
$n_{\mathrm{arr}}(k)$ & 到达条件连续计数变量 \\
$n_{\mathrm{lea}}(k)$ & 离开条件连续计数变量 \\
$T_d$ & 防断裂延时点数 \\
$\mathbf{B}_0$ & 事件片段的基线估计向量 \\
$N_{\mathrm{pre}}$ & 到达前基线估计窗口长度 \\
$\Delta\mathbf{B}(n)$ & 基线扣除后的三轴扰动向量（第3章） \\
...
$\overline{\mathbf{B}}(k)$ & 稳定窗均值向量（第4章） \\
...
$\mathbf{S}_{\mathrm{fix}}$ & 退化分支近似稳态向量 \\
$\Delta\mathbf{S}$ & 占用参考相对环境参考的漂移向量（第4章） \\
$\Delta S$ & 漂移向量 $\Delta\mathbf{S}$ 的幅值 \\
$\mathrm{dist}$ & 新稳定点漂移与当前占用漂移之间的相似性距离 \\
$D_{\mathrm{th}}$ & 停车漂移阈值 \\
