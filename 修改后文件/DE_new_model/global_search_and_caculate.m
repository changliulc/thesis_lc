...
%     lower_bound((i_temp-1)*3+1) = -5000;%原本是-500
%     upper_bound((i_temp-1)*3+1) =  5000;%原本是500
...
%%
% GlobalSearch算法
% F=@(t)Optim_obj_fun(t(1),t(2),t(3),t(4),t(5),t(6),t(7),t(8),t(9),t(10),t(11));%y0未知
F=@(t)cost_function(t,data_choose,y_begin);%y0已知

%分别是6个磁偶极矩，z0,dx,dy,dz
theta=[-134.928121845347 -229.30014864253 302.81227776453 500 103.806541436847 -500 -1.17182741768823 0.288322879919281 0.31 -0.31];
%小型车
lb=lower_bound;%未知数的约束下界lb  约束上界ub
ub=upper_bound;

%中型货车
% lb=[-1500*ones(1,6),-2.25,-3,3,0,0];
% ub=[1500*ones(1,6),-0.75,0,3.5,0.3,0.5];
options = optimset('Algorithm', 'SQP', ...
                   'Display', 'iter', ...  % 显示每次迭代的信息
                   'MaxIter', 100, ...    % 最大迭代次数
                   'MaxFunEvals', 100000, ... % 目标函数最大评估次数
                   'TolFun', 1e-6, ...     % 目标函数容差
                   'TolX', 1e-6);          % 解的容差
                    
problem=createOptimProblem('fmincon','objective',F,'x0',theta,'lb',lb,'ub',ub,'options',optimset);
% gs=GlobalSearch; disp(gs)
global iteration_count ; iteration_count=0;
gs = GlobalSearch('NumTrialPoints', 1000, ... % 总试验点的数量
                  'NumStageOnePoints', 200, ... % 第一阶段的试验点数量
                  'StartPointsToRun', 'all', ... % 启动局部搜索的试验点
                  'OutputFcn', @(optimValues, state) myOutputFcn(optimValues, state));
...
% 
% % 分别是6个磁偶极矩，z0,dx,dy,dz
% theta = [528.681340356105	634.518060124140	-1497.81638881467	-848.367013982473	-800.671021369028	1373.63863641212	0.0485181456102382	-0.0437576343556945	0.288309490047450	0.308801309008699];
% 
% % 小型车
% lb = lower_bound; % 未知数的约束下界 lb 约束上界 ub
% ub = upper_bound;
% 
% % 模拟退火算法选项
...
%                        'MaxFunctionEvaluations', 1000000, ... % 目标函数最大评估次数
%                        'FunctionTolerance', 1e-6, ...  % 目标函数容差
%                        'TolFun', 1e-6, ...    % 目标函数容差
%                        'InitialTemperature', 10000, ... % 初始温度
%                        'TemperatureFcn', @temperatureexp, ... % 温度函数
%                        'ReannealInterval', 50000, ... % 重新退火间隔
%                        'StallIterLimit', 100); % 停滞迭代次数限制
% 
% % 使用模拟退火算法进行优化
% [theta, fval, exitflag, output] = simulannealbnd(F, theta, lb, ub, options);
% 
% % 输出优化结果
...
%%
% th=[-246.829309540046 358.948220935425 -22.1844831863747 149.621794157071 -500 -241.53491331406 -1.284723064022 0.1 0.262948516380235 0.293112328621707];
