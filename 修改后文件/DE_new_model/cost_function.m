function [score] = cost_function(population,data_raw,y_begin)
...
% res=(data(:,1)-Bxpre).^2+(data(:,2)-Bypre).^2;
score=sum(res.^0.5)/L;
% res=max([sum(abs(data(:,1)-Bxpre)),sum(abs(data(:,2)-Bypre)),sum(abs(data(:,3)-Bzpre))]);
% score=res/L;
