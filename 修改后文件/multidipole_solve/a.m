clc;close all;clear;
%数据读取
[data_left_num,~,data_left]=xlsread('D:\Document\lab_office\差分-车型分类\磁偶极子原始车辆数据\2024.6.11车辆数据.xls',4,'A1:J116');
arrive_status=2;
leave_status=4;
...
F=@(t)b(t(1),t(2),t(3),t(4),t(5),t(6),t(7),t(8),t(9),t(10));%y0已知

theta=[60,100,-50,60,100,-50,-0.8,2,0,0];%分别是6个磁偶极矩，z0,dx,dy,dz
%小型车
lb=[-500*ones(1,6),-1.5,2,-0.2,0];%未知数的约束下界lb  约束上界ub
ub=[500*ones(1,6),-0.5,2.6,0.2,0.2];
...
th=theta;
%%
th = [-3.5771897072448127, -103.47688587580326, -93.80731829725411, -19.129858067230433, 2.7735443480720647, -11.950304817050267, -0.8470420420095162, 1.5438628878950846, -0.036083030054685744, 0.2524447414727915];
m=[th(1:3);th(4:6);];%给定初始值 单位是Am² 需要迭代计算的值
[n,c]=size(m);
x10=-v*T*L/2;y0=2.2;z0=th(7);%给定的y0从视频中观察得到
